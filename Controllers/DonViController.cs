...
namespace ERP.Controllers
{
    ...
    public class DonViController : ControllerBase
    {
        ...
        public DonViController(IConfiguration _configuration, IUnitofWork _uow, UserManager<ApplicationUser> _userManager)
        {
            uow = _uow;
        ...
        public ActionResult Delete(Guid id)
        {
            lock (Commons.LockObjectState)
            {
                ...
                if (
                    uow.DonViTinhs.Exists(x => !x.IsDeleted && x.DonVi_Id == id)
                    || uow.Menus.Exists(x => !x.IsDeleted && x.DonVi_id == id)
                    || uow.phienBans.Exists(x => !x.IsDeleted && x.DonVi_Id == id)
                    || uow.phongbans.Exists(x => !x.IsDeleted && x.DonVi_Id == id)
                    || uow.PhongBanHRMs.Exists(x => x.DonVi_Id == id)
                    || uow.ThongBaoHeThongs.Exists(x => x.DonVi_Id == id)
                )
                {
                    return StatusCode(StatusCodes.Status409Conflict, "Đơn vị đã được sử dụng không thể xóa!");
                }
        ...
        public ActionResult ExportFileExcel()
        {
            ...
            using ExcelPackage package = new(new FileInfo("Uploads/Templates/Danhmucdonvi.xlsx"));
            ...
            foreach (var item2 in donvi)
            {
                ...
                worksheet2.Cells["B" + indexrow2].Value = item2.MaDonVi;
                worksheet2.Cells["C" + indexrow2].Value = item2.TenDonVi;
                worksheet2.Cells["D" + indexrow2].Value = item2.TapDoan.TenTapDoan;
                worksheet2.Cells["E" + indexrow].Value = GetTenDonVi(item2?.DonVi_Id);
        ...
        [HttpGet("don-vi-by-user")]
        public ActionResult GetDonViByUser()
        {
            dbAdapter.connect();
