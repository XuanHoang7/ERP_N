    ...
    public class BoPhanController : ControllerBase
    {
        ...
        public ActionResult Get(Guid id)
        {
            var query = uow.BoPhans.GetAll(x => x.Id == id).Select(x => new
            {
                ...
                x.BoPhan_Id,
                x.PhongBan_Id,
                DonVi_Id = uow.phongbans.GetById(x.PhongBan_Id).DonVi_Id
            }).FirstOrDefault();
            if (query == null)
            {
                return NotFound();
        ...
        {
            var check = uow.phongbans.GetAll(x=>x.DonVi_Id == donviid).Select(x=>x.Id).ToList();
            var rootBoPhanList = uow.BoPhans.GetAll(x => !x.IsDeleted && x.BoPhan_Id == null && (donviid == null || check.Contains(x.PhongBan_Id.Value)) && (phongBan_Id == null || x.PhongBan_Id == phongBan_Id)).ToList();
