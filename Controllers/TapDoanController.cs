    ...
    public class TapDoanController : ControllerBase
    {
        ...
        public ActionResult Delete(Guid id)
        {
            lock (Commons.LockObjectState)
            {
                ...
                if (
                    uow.DonVis.Exists(x => !x.IsDeleted && x.TapDoan_Id == id)
                    || uow.Menus.Exists(x => !x.IsDeleted && x.TapDoan_Id == id)
                )
                {
                    return StatusCode(StatusCodes.Status409Conflict, "Tập đoàn đã được sử dụng không thể xóa!");
                }
